 ...
 */
class SpecialEquipmentQuery extends SpecialEquipment
{
    ...
    public $priceFrom;
    public $priceTo;
    public $type_id = Car::CAR_TYPE_SPECIAL_EQUIPMENT;
    public $car_model;
    ...
    {
        return [
            [['id', 'sq_category_id', 'type_id', 'marka_id', 'model_id', 'wheel_id', 'kuzov_id', 'privod_id', 'transmission_id', 'engine_id', 'color_id', 'state_id', 'region_id', 'run_length', 'created_by', 'updated_by'], 'integer'],
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            ...
            'rent' => ( isset($params['rent']) && $params['rent'] ) ? true : false,
        ...
        $query->andWhere('price > '. ($this->priceFrom ?: 100000).' AND price < '. ($this->priceTo ?: 40000000));
         ...
         $query->joinWith(['carModel' => function ($q) {
                 ...
                 $q->joinWith(['marka' => function($qi) {
                         $qi->where(['markas.car_type_id' => Car::CAR_TYPE_SPECIAL_EQUIPMENT]);
                     }
                         ], true, 'INNER JOIN');
                 }], true, 'INNER JOIN');
    ...
    public function attributeLabels()
    {
        return [
            ...
            'model_name' => Yii::t('app', 'Special equipment model'),
        ];
    }
}